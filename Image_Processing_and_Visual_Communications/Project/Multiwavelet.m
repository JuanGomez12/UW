...
[A,map] = imread("017_MRI.gif",1); 
A_RGB = ind2rgb(A,map);

% Read SPECT image and convert it to RGB format %
[B,map] = imread("017_PET.gif",1);
B_RGB = ind2rgb(B,map);
...
LL_LL_F = 0.5*(LL_LL_X+LL_LL_Y);
LL_LH_F = 0.5*(LL_LH_X+LL_LH_Y);
LL_HL_F = 0.5*(LL_HL_X+LL_HL_Y);
LL_HH_F = 0.5*(LL_HH_X+LL_HH_Y);
...
% Compute IFPM %
ifpm = IFPM(im2uint8(A_RGB), im2uint8(B_RGB), im2uint8(F));
